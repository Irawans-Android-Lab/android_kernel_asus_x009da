...
/* ---[5833][Sensors][JasonHsing] Reducing G-sensor Resume Time_3 20160705 Begin ---*/
#include <linux/time.h>
/* ---[5833][Sensors][JasonHsing] 20160705 End ---*/
...
#define MXC400X_VDD_MIN_UV	1750000
#define MXC400X_VDD_MAX_UV	1950000

/* --- [5833][Sensor][LewisChen] Mexic G-Sensor orientation . 20160324 Begin ---*/
#if defined (CONFIG_BSP_HW_SKU_5833)
#define INSTALL_DIR 4
#else
#define INSTALL_DIR 7
#endif
/*--- [5833][Audio][LewisChen] 20160324 End  ---*/

static int sensor_layout = INSTALL_DIR;
...
/* ---[5833][Sensors][JasonHsing] Reducing G-sensor Resume Time_3 20160705 Begin ---*/
static struct timeval enable_time;
static struct timeval report_time;
/* ---[5833][Sensors][JasonHsing] 20160705 End ---*/
...
//	unsigned int status;
	...
	int rc = 0;
/* ---[5833][Sensors][JasonHsing] Reducing G-sensor Resume Time_4 20160707 Begin ---*/
	unsigned int inval;
	mutex_lock(&memsic->ecompass_lock);
	
	do_gettimeofday(&report_time);

	inval = (report_time.tv_sec - enable_time.tv_sec) * 1000000 + report_time.tv_usec - enable_time.tv_usec;

	if (inval < 300*1000)
	{
		msleep(300 - inval/1000);
		printk("enter sleep func inval=%d\n",inval);
	}
/* ---[5833][Sensors][JasonHsing] 20160707 End ---*/
...
/* ---[5833][Sensors][LewisChen]   ID judgment for MXC4005XC and MXC4005XCXE. 20160525 Begin ---*/
	//if (data != MXC400X_ID)
	if ((data != MXC400X_ID_1) && (data != MXC400X_ID_2)){
		dev_warn(&memsic->i2c->dev, "Chip id %d is wrong\n", data);
		return -ENODEV;
	}
		...
		unsigned int enable)
{
   ...
   // printk("memsic enable  ok!!! memsic->enable = %d\n", memsic->enable);
...
/* ---[5833][Sensors][JasonHsing] Reducing G-sensor Resume Time_2 20160623 Begin ---*/
        do_gettimeofday(&enable_time);
/* ---[5833][Sensors][JasonHsing] 20160623 End ---*/
       // printk("memsic delay 300ms power ok!!!\n");
	...
	switch (cmd) {
		case MXC400X_IOCTL_READ_REG:
			...
			if (rc) {
				...
				//printk("memsic read reg = 0x%x\n", reg_addr);
			}
			if (copy_to_user(pa, &reg_value, sizeof(reg_value))) {
				return -EFAULT;
			} else {
				//printk("memsic read reg value = 0x%x\n", reg_value);
			}
			break;
		case MXC400X_IOCTL_WRITE_REG:
			...
			if (rc) {
				dev_err(&mxc400x_data_struct->i2c->dev, "write reg %d failed at %d.(%d)\n",
						data[1], __LINE__, rc);
				printk(" memsic write reg %d failed at %d.(%d)\n",
						data[1], __LINE__, rc);
				return -EFAULT;
			} else {
				//printk("memsic write reg = 0x%x value = 0x%x \n", data[0], data[1]);
			}
			break;
		...
		case MXC400X_IOCTL_GET_LAYOUT: 
			...
			if(copy_to_user(pa, &(mxc400x_data_struct->dir), sizeof(mxc400x_data_struct->dir)))
			{    
				return -EFAULT;
				break;
			}    
