...
static int mmc3524x_check_device(struct mmc3524x_data *memsic)
{
	...
	if (data != MMC3524X_PRODUCT_ID){
		dev_err(&memsic->i2c->dev, "wrong MMC3524X_PRODUCT_ID = %d \n", data);
		return -ENODEV;
	}
	return 0;
}
...
//	memsic->dir = i;
//<--[5830][Sensors][JasonHsing] Change e-compass direction 20160225 BEGIN --
        memsic->dir = 5;
//-->[5830][Sensors][JasonHsing] Change e-compass direction 20160225 END --
...
//<--[5830][Sensors][JasonHsing] Add self-test function for e-compass 20160309 BEGIN --
static ssize_t mmc3524x_selftest(struct device *dev, 
				struct device_attribute *attr, char *buf)
{
	unsigned char data[16] = {0};	
	unsigned int status;
	...
	int res, res1, res2;
	res = 1;
	//mutex_lock(&mc3524x_data_struct->ecompass_lock);
	// Power On check 

	data[0] = MMC3524X_REG_CTRL;
	data[1] = MMC3524X_CTRL_TM;
	if (regmap_write(mmc3524x_data_struct->regmap,data[0],data[1])){
	...
	rc = regmap_read(mmc3524x_data_struct->regmap, data[0], &status);
	if (rc) {
		printk("[MEMSIC] Self_Test: Power On check Fail");
		res=0;
	}
	...
	while (!(status & 0x01)) {
		...
		rc = regmap_read(mmc3524x_data_struct->regmap, MMC3524X_REG_DS, &status);
		if (rc) {
			printk("[MEMSIC] Self_Test: Power On check Fail");
			res=0;
		break;
	...
	rc = regmap_bulk_read(mmc3524x_data_struct->regmap, MMC3524X_REG_DATA, data, 6);
	if (rc) {
		printk("[MEMSIC] Self_Test: Power On check Fail");
		res=0;
	}
	...
	vec[2] = 65536 - vec[2];
	if((vec[0] < 65536&&vec[0] > 0) && (vec[1] < 65536&&vec[1] > 0) && (vec[2] < 65536&&vec[2] > 0))
		res2 = 1;
	else{
	...
	//mutex_unlock(&&mc3524x_data_struct->ecompass_lock);
	return sprintf(buf, 
	"PowerOn=%d, RoughOffset=%d, overall=%d\n[X:%04x][Y:%04x][Z:%04x]\nself_test: %s\n", 
	res1, res2, res, vec[0], vec[1], vec[2], (res)?"Pass":"Fail");
}
//-->[5830][Sensors][JasonHsing] Add self-test function for e-compass 20160309 END --
...
//<--[5830][Sensors][JasonHsing] Add self-test function for e-compass 20160309 BEGIN --
static DEVICE_ATTR(self_test, 0444, mmc3524x_selftest, NULL);
//-->[5830][Sensors][JasonHsing] Add self-test function for e-compass 20160309 END --
